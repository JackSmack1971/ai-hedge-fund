...
def calculate_fcf_volatility(fcf_history: list[float]) -> float:
    ...
    except (TypeError, ValueError, statistics.StatisticsError, ZeroDivisionError) as exc:
        logger.debug("FCF volatility fallback used after calculation error: %s", exc, exc_info=True)
        return 0.5


def calculate_enhanced_dcf_value(
    fcf_history: list[float], growth_metrics: dict, wacc: float, market_cap: float, revenue_growth: float | None = None
) -> float:
