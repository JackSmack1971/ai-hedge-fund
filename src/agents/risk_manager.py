...
def risk_management_agent(state: AgentState, agent_id: str = "risk_management_agent"):
    ...
    active_positions = {
        t for t, pos in portfolio.get("positions", {}).items() if abs(pos.get("long", 0) - pos.get("short", 0)) > 0
    }
    ...
    for ticker in tickers:
        ...
        progress.update_status(
            agent_id, ticker, f"Adj. limit: {combined_limit_pct:.1%}, Available: ${max_position_size:.0f}"
        )
